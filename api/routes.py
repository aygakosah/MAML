...
import jwt
import pymysql
from .models import db, Users, JWTTokenBlocklist, Pages
...
def maml_to_html(username, pageName, data):
    ...
    for jsonString in jsonArray:
        ...
        if (jsonObject["type"] == "rect"): 
            ...
            html += "<button style='position:absolute; width:%dpx; height:%dpx; left: %dpx; top: %dpx'>\n%s</button>\n" % (jsonObject["w"], jsonObject["h"], jsonObject["x"], jsonObject["y"], jsonObject["txt"])
...
class UploadPage(Resource):
    ...
    def post(self, current_user):
        ...
        page_Info = Pages(pageId=pageName, userId= self.id, created_At= datetime.now(timezone.utc), stateJSON=jsonState, textMap=textMap)
        page_Info.save()
...
# the unique name chosen by the user for that page      
